...
export const CartItem: React.FC<Props> = ({ line, updateQuantity, removeFromCart, compact = false }) => {
  return (
    <div
      className={cn(
        "flex min-w-0 items-center justify-between gap-2 border-b border-gray-100 last:border-b-0",
        compact ? "flex-nowrap py-1.5" : "flex-wrap py-2.5",
      )}
    >
      <div className={cn("min-w-0 flex-1", !compact && "basis-32")}>
      ...
      <div className="flex shrink-0 items-center gap-1">
        <Button
          type="button"
          variant="secondary"
          className={cn("rounded-lg p-0", compact ? "size-9" : "size-7")}
          onClick={() => updateQuantity(line.clientKey, line.quantity - 1)}
        >
          <FontAwesomeIcon icon={faMinus} className={compact ? "size-3" : "size-2.5"} aria-hidden />
        </Button>
        ...
        <Button
          ...
          onClick={() => updateQuantity(line.clientKey, line.quantity + 1)}
        >
          <FontAwesomeIcon icon={faPlus} className={compact ? "size-3" : "size-2.5"} aria-hidden />
        </Button>
        <Button
          type="button"
          variant="ghost"
          className={cn(
            "rounded-lg p-0 text-red-600 hover:bg-red-50 hover:text-red-700",
            compact ? "size-9" : "size-7",
          )}
          onClick={() => removeFromCart(line.clientKey)}
        >
          <FontAwesomeIcon icon={faTrash} className={compact ? "size-3.5" : "size-3"} aria-hidden />
        </Button>
      </div>
    </div>
  )
