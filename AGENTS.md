...
Env (Vite): `VITE_API_URL`, `VITE_OAUTH_CLIENT_ID` (must match the Rails “Restaurant App” OAuth client). Optional `VITE_CLEANTER_URL` (default `http://localhost:9200`) for the Android Cleanter print bridge. Cookies need API CORS + `CROSS_ORIGIN_COOKIES` aligned with the SPA origin.
